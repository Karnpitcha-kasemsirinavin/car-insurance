...
import {
    TextField, Button, Table, TableBody, TableCell, TableHead, TableRow, IconButton, Paper, TablePagination, MenuItem, Select, InputLabel, FormControl, Tooltip
} from '@mui/material';
...
const PackageTable = ({ title, table }) => {
    ...
    const handleMiniStatusToggle = () => {
        if (editedRow) {
            const newStatus = editedRow.miniStatus === 1 ? 0 : 1;
            setEditedRow({ ...editedRow, miniStatus: newStatus });
        }
    };
    ...
    return (
        <div className='all-container'>
            <div className='table-section'>
                ...
                <Paper className="users-table-root" style={{ overflowX: 'auto' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }} className='search-container-table'>
                        <div style={{ display: 'flex', gap: '10px' }}>
                            ...
                            <TextField variant="outlined" placeholder="ค้นหา..." value={searchValue} onChange={handleSearchValueChange} />
                        </div>
                        <FormControl variant="outlined">
                                ...
                                <TableBody>
                                    {filteredData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, index) => (
                                        <TableRow key={index}>
                                            {fields.map((field) => (
                                                <TableCell key={field.field}>
                                                    <Tooltip title={row[field.field]?.toString() || ''}>
                                                        {editingRow === index ? (
                                                            field.field === 'Status' ? (
                                                                <Button onClick={handleStatusToggle}> {/* Toggle status on button click */}
                                                                    {editedRow && editedRow.Status === 1 ? <CheckIcon /> : <CloseIcon />}
                                                                </Button>
                                                            ) : field.field === 'miniStatus' ? (
                                                                <Button onClick={handleMiniStatusToggle}> {/* Toggle status on button click */}
                                                                    {editedRow && editedRow.miniStatus === 1 ? <CheckIcon /> : <CloseIcon />}
                                                                </Button>
                                                            ) :
                                                            <TextField
                                                                value={editedRow[field.field] || ''}
                                                            ...
                                                            <Button>
                                                                {row.miniStatus === 1 ? <CheckIcon /> : <CloseIcon />}
                                                            </Button>
                                                        ) : (
                                                            row[field.field]
                                                        )}
                                            ...
                                            <TableCell>
                                                {editingRow === index ? (
                                                    <>
                                                        <IconButton onClick={confirmSave}><SaveIcon /></IconButton>
            ...
            <Popup isOpen={isPopupOpen} onClose={handleClosePopup} onConfirm={confirmSave} />
            <ErrorPopup isOpen={isErrorPopupOpen} onClose={() => setIsErrorPopupOpen(false)} message={errorMessage} />
        </div>
    );
};
